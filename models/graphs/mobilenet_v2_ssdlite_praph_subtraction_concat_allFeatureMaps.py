...
def mobilenet_v2_ssdlite(input_image, sub):
    ...
    # stage5 19*19*96 -> 10*10*160 (link 1) 
    ...
    x = KL.concatenate([x, sub] , axis=-1)#x = KL.Add(name='10_10_160stage5_add')([x, sub])
    
    x = _inverted_res_block(x, filters=160, alpha=alpha, stride=1,
                           expansion=1, stage='sub', block_id=1) # compress the channels from 320 to 160 in case the param size won't be so big, that the memory exceed
